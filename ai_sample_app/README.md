...
5. `[Optional]` Configure CometChat credentials:
    - Open the `app_credentials.dart` file located at `ai_sample_app/lib/app_credentials.dart` and enter your CometChat _`App ID`_, _`Region`_, and _`Auth Key`_:
      ```dart
      AppCredentials {
...
6. In the Run/Debug Configurations dropdown (on the top toolbar), select the preconfigured `sample_app_ai` run configuration. Connect a `physical device` or select an `emulator` from the Device Selector dropdown. Click the Run ▶ button to launch the app.
