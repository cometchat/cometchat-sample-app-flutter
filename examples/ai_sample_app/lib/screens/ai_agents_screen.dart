...
import 'ai_messages_screen.dart';
...
class _AiAgentsScreenState extends State<AiAgentsScreen> {
  ...
  Widget build(BuildContext context) {
    return CometChatUsers(
      ...
      usersRequestBuilder: UsersRequestBuilder()..roles = [AIConstants.aiRole],
      onItemTap: (ctx, user) => Navigator.push(ctx,
        MaterialPageRoute(builder: (_) => AiMessagesScreen(user: user))),
      appBarOptions: (context) => [_buildProfileMenu()],
    );
  }
  ...
  Widget _buildProfileMenu() {
    ...
    return PopupMenuButton<String>(
      ...
      elevation: 4,
      icon: Padding(
        padding: EdgeInsets.only(left: _spacing.padding3 ?? 0, right: _spacing.padding4 ?? 0),
        child: CometChatAvatar(width: 40, height: 40,
          image: loggedInUser?.avatar ?? '', name: loggedInUser?.name ?? '')),
      onSelected: (value) async {
        if (value == '/logout') {
          await CometChatUIKit.logout(
            onSuccess: (_) => Navigator.pushAndRemoveUntil(context,
              MaterialPageRoute(builder: (_) => const LoginScreen()), (route) => false),
            onError: (e) => debugPrint('Logout error: ${e.message}'));
        }
      },
      position: PopupMenuPosition.under,
      itemBuilder: (BuildContext bc) => [
        PopupMenuItem(height: 44, padding: EdgeInsets.all(_spacing.padding4 ?? 16), enabled: false,
          child: Row(children: [
            Padding(padding: EdgeInsets.only(right: _spacing.padding2 ?? 8),
              child: Icon(Icons.account_circle_outlined, color: _colorPalette.iconSecondary, size: 24)),
        ...
        PopupMenuItem(height: 44, padding: EdgeInsets.all(_spacing.padding4 ?? 16), value: '/logout',
          child: Row(children: [
            Padding(padding: EdgeInsets.only(right: _spacing.padding2 ?? 8),
              child: Icon(Icons.logout, color: _colorPalette.error, size: 24)),
