...
class InMemoryChatController
  ...
  Future<void> insertAllMessages(
    ...
    // Filter out duplicates within the incoming list
    final seenIds = <String>{};
    final uniqueIncoming = <Message>[];
    for (final message in messages) {
      if (!seenIds.contains(message.id)) {
        seenIds.add(message.id);
        uniqueIncoming.add(message);
      }
    }

    // Filter out messages that already exist in the controller
    final existingIds = _messages.map((m) => m.id).toSet();
    final messagesToInsert = uniqueIncoming
        .where((m) => !existingIds.contains(m.id))
        .toList();

    if (messagesToInsert.isEmpty) return;
