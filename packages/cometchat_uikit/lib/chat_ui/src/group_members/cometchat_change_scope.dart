...
class _CometChatChangeScopeState extends State<CometChatChangeScope> {
  ...
  void didChangeDependencies() {
    ...
    style = CometChatThemeHelper.getTheme<CometChatChangeScopeStyle>(
      context: context,
              ...
              children: [
                      ...
                      style: ButtonStyle(
                        ...
                        shape: WidgetStateProperty.all(
                          RoundedRectangleBorder(
                            ...
                            borderRadius:
                                style.cancelButtonBorderRadius ??
                                BorderRadius.all(
                ...
                Expanded(
                  child: TextButton(
                    ...
                    style: ButtonStyle(
                      backgroundColor: WidgetStateProperty.all(
                        style.saveButtonBackgroundColor ??
                            ((widget.member.scope == selectedScope)
                                ? colorPalette.neutral300
                                : colorPalette.buttonBackground),
                      ),
                      side: WidgetStateProperty.all(
                        style.saveButtonBorder ??
                            BorderSide(
                              color:
                                  ((widget.member.scope == selectedScope)
                                      ? colorPalette.neutral300
  ...
  Widget _buildScopeTile(
    int index,
    CometChatChangeScopeStyle style,
    ...
    final tile = Container(
      ...
      decoration: BoxDecoration(
        color: isSelected
            ? (style.selectedTileColor ?? colorPalette.borderLight)
            : (style.tileColor ?? colorPalette.background1),
        borderRadius: BorderRadius.only(
          topLeft: index == 0
              ? Radius.circular(spacing.radius2 ?? 0)
              : Radius.zero,
          topRight: index == 0
              ? Radius.circular(spacing.radius2 ?? 0)
              : Radius.zero,
          bottomLeft: index == scopes.length - 1
              ? Radius.circular(spacing.radius2 ?? 0)
              : Radius.zero,
          bottomRight: index == scopes.length - 1
              ? Radius.circular(spacing.radius2 ?? 0)
              : Radius.zero,
        ),
      ),
      child: ListTile(
        title: Text(
          ...
          style:
              TextStyle(
                ...
                isSelected
                    ? style.selectedScopeTextStyle
                    : style.scopeTextStyle,
              ),
        ),
        contentPadding: EdgeInsets.symmetric(horizontal: spacing.padding4 ?? 0),
        // minLeadingWidth: 0,
        ...
        trailing: Transform.scale(
          ...
          child: Radio(
            ...
            // ignore: deprecated_member_use  (RadioGroup requires Flutter 3.32+, above this package's floor)
            groupValue: selectedScope,
            fillColor: WidgetStatePropertyAll(
              isSelected
                  ? (style.radioButtonSelectedColor ??
                        colorPalette.borderHighlight)
                  : (style.radioButtonColor ?? colorPalette.borderDefault),
            ),
