...
class _CometChatPollsBubbleState extends State<CometChatPollsBubble> {
  ...
  List<Widget> _getAvatars(List<User> voters, BubbleAlignment? alignment, CometChatColorPalette colorPalette, CometChatTypography typography, CometChatPollsBubbleStyle? style) {
  ...
  Widget _buildRadio(int index, String id, String value, String chosenId, String pollId, int votes, int totalVotes, List<User> voters,
            ...
            Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: isVoting
                      ? Colors.transparent
                      : (id == chosenId
                          ? style.selectedOptionColor ?? (widget.alignment == BubbleAlignment.right ? colorPalette.white : colorPalette.primary)
                          : style.unSelectedOptionColor) ?? style.radioButtonColor,
                ),
                child: isVoting
                    ? SizedBox(
                        height: 20,
                    ...
                    : id == chosenId
                        ? Icon(Icons.check, size: 14, color: style.iconColor ?? (widget.alignment == BubbleAlignment.right ? colorPalette.primary : colorPalette.white))
                        : null,
            ),
            Flexible(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                      ...
                      children: [
                        Flexible(
                          child: Text(
                            value,
                            style:
                                TextStyle(color: widget.alignment == BubbleAlignment.right ? colorPalette.white : colorPalette.neutral900, fontSize: typography.body?.regular?.fontSize, fontWeight: typography.body?.regular?.fontWeight)
                          ...
                          children: [
                            if (votes > 0) SizedBox(
                              child: Row(
                                ...
                                children: _getAvatars(voters, widget.alignment, colorPalette, typography, style),
                              ),
                            ),
                            if (votes > 0) Padding(
                              ...
                              child: Text(
                                "$votes",
                                style: TextStyle(color: widget.alignment == BubbleAlignment.right ? colorPalette.white : colorPalette.neutral900, fontSize: typography.body?.regular?.fontSize, fontWeight: typography.body?.regular?.fontWeight)
                  ...
                  LinearProgressIndicator(
                    color: style.progressColor ?? (widget.alignment == BubbleAlignment.right ? colorPalette.white : colorPalette.primary),
                    backgroundColor: style.progressBackgroundColor ?? (widget.alignment == BubbleAlignment.right ? colorPalette.extendedPrimary700 : colorPalette.neutral400),
                    minHeight: 8,
                    value: percentage,
  ...
  Widget _buildPollWidget(List<PollOptions> options, String chosenId,
    ...
    return Column(
      children: [
        for (int index = 0; index < options.length; index++)
          _buildRadio(index, options[index].id, options[index].optionText,
          ...
          Padding(
            ...
            child: Text(
              ...
              style: TextStyle(
                      ...
                      color: widget.alignment == BubbleAlignment.right ? colorPalette.white : colorPalette.neutral900)
                  .merge(style.questionTextStyle),
            ),
          ),
          _buildPollWidget(options, chosenId, widget.pollId ?? '', totalVotes, style, colorPalette, spacing, typography),
        ],
      ),
    );
  }
