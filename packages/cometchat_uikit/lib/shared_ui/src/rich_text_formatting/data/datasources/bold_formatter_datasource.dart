...
///
/// This formatter handles bold formatting in Markdown style:
/// - Pattern: `**text**`
/// - Opening marker: `**`
/// - Closing marker: `**`
...
///
/// Examples:
/// - `**hello**` renders as bold "hello"
/// - Applying bold to "world" produces `**world**`
/// - Applying bold with no selection inserts `**bold text**`
class BoldFormatterDataSource implements FormatterDataSource {
  @override
  ...
  @override
  String get placeholderText => 'bold text';
  ...
  /// Mention spans are left untouched.
  FormatterResult _applyFormatAroundMentions({
    ...
    // Build segments of non-mention text within the selection
    ...
    for (final mention in mentionRanges) {
      ...
      // Clamp mention bounds to selection
    ...
    for (final segment in segments) {
      ...
      if (segment.isMention) {
        ...
        // Only wrap non-empty, non-whitespace-only segments
        final trimmed = segmentText.trim();
        if (trimmed.isEmpty) {
          buffer.write(segmentText);
        } else {
          // Preserve leading/trailing whitespace outside markers
  ...
  bool isActiveAt(String text, int position) {
    ...
    // Check all matches to see if position is inside any bold text
  ...
  List<AttributedTextData> getAttributedText(String text) {
    ...
    for (final match in matches) {
      // Apply bold attribute to the entire match (including markers)
