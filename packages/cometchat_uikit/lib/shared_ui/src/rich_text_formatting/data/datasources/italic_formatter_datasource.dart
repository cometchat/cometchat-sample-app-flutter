...
/// Formatter data source for italic text using *text* or _text_ syntax.
///
/// This formatter handles italic formatting in Markdown style:
...
///
/// Examples:
/// - `*hello*` renders as italic "hello"
/// - `_hello_` renders as italic "hello"
/// - Applying italic to "world" produces `*world*`
/// - Applying italic with no selection inserts `*italic text*`
class ItalicFormatterDataSource implements FormatterDataSource {
  @override
  RegExp get pattern => RegExp(r'(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)|_(.+?)_');
  ...
  @override
  String get placeholderText => 'italic text';
  ...
  FormatterResult applyFormat({
    ...
    if (selection.isCollapsed) {
      ...
      // Check for both * and _ markers
      final isWrappedWithAsterisk =
          beforeSelection == openingMarker && afterSelection == closingMarker;
      final isWrappedWithUnderscore =
          beforeSelection == '_' && afterSelection == '_';

      if (isWrappedWithAsterisk || isWrappedWithUnderscore) {
        // Remove italic markers (toggle off)
        final markerLength = isWrappedWithAsterisk
  ...
  bool isActiveAt(String text, int position) {
    ...
    // Check all matches to see if position is inside any italic text
  ...
  List<AttributedTextData> getAttributedText(String text) {
    ...
    for (final match in matches) {
      // Apply italic attribute to the entire match (including markers)
