...
class MessageUtils {
  ...
  static Widget? _getStatusInfoView(
    ...
    if (template?.statusInfoView != null) {
      ...
      return Row(
        ...
        children: [
          Container(
            padding: EdgeInsets.only(left: 0, bottom: spacing.padding1 ?? 0),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.end,
              mainAxisSize: MainAxisSize.min,
              children: [
                // See the clean_architecture copy: the tag applies to any edited
                // `message`-category message (text + media captions), not just
                // text.
  ...
  static Widget _getName(
    ...
    // Use Flexible instead of MediaQuery-based width to avoid rebuilds during keyboard animation
    return Flexible(
      child: Container(
