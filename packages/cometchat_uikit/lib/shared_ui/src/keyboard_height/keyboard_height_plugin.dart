...
typedef KeyboardHeightCallback =
    void Function(double keyboardHeight, double safeAreaBottom);
...
class KeyboardHeightPlugin {
  static const EventChannel _keyboardHeightEventChannel = EventChannel(
    'com.cometchat.keyboard_height_channel',
  );
  ...
  void onKeyboardHeightChanged(KeyboardHeightCallback callback) {
    ...
    _listenerCount++;
    _keyboardHeightSubscription = _getSharedStream().listen(
      (dynamic data) {
        if (data is Map) {
          final keyboardHeight =
              (data['keyboardHeight'] as num?)?.toDouble() ?? 0.0;
          final safeAreaBottom =
              (data['safeAreaBottom'] as num?)?.toDouble() ?? 0.0;
