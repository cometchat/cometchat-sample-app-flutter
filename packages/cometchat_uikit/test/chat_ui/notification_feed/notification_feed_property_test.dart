...
List<NotificationCategory> _randomCategories(int count) {
  return List.generate(
    count,
    (i) => NotificationCategory(id: 'cat_$i', label: 'Category $i'),
  );
}
...
void main() {
  ...
  group('Property 2: grouping correctness', () {
    test('for 100 random item lists, grouping preserves all items', () {
      for (int iteration = 0; iteration < 100; iteration++) {
        ...
        final totalGrouped = groups.fold<int>(
          0,
    ...
    test(
      'for 100 random item lists, items within groups are sorted newest-first',
      () {
        for (int iteration = 0; iteration < 100; iteration++) {
          ...
          for (final group in groups) {
            for (int i = 0; i < group.items.length - 1; i++) {
              expect(
                group.items[i].sentAt >= group.items[i + 1].sentAt,
                true,
                reason:
  ...
  group('Property 4: unread count invariant', () {
    test(
      'for 100 random item lists, unread count matches items with null readAt',
      () async {
        for (int iteration = 0; iteration < 100; iteration++) {
    ...
    test(
      'for 100 random item lists, per-category unread counts are correct',
      () async {
        for (int iteration = 0; iteration < 100; iteration++) {
          ...
          for (final entry in expectedCounts.entries) {
            expect(
              bloc.state.categoryUnreadCounts[entry.key],
              entry.value,
              reason:
  ...
  group('Property 5: delivery on fetch/receive', () {
    test(
      'for 100 random item lists, markAsDelivered called for each item',
      () async {
        for (int iteration = 0; iteration < 100; iteration++) {
  ...
  group('Property 10: pagination exhaustion', () {
    test(
      'for 100 iterations, hasMorePages becomes false after empty page',
      () async {
        for (int iteration = 0; iteration < 100; iteration++) {
    ...
    test('does not fetch when already exhausted', () async {
      for (int iteration = 0; iteration < 100; iteration++) {
        ...
        expect(
          ...
          reason: 'Iteration $iteration: should not fetch after exhaustion',
        );
