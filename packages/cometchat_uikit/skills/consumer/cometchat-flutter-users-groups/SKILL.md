...
- `CometChatGroupMembers` still uses a legacy GetX controller internally (`cometchat_group_members_controller.dart`), not BLoC. The public API is the same, but if you need to extend the controller, be aware it's `GetxController` not `Bloc`.
